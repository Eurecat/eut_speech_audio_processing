...
from audio_stream_manager.save_mp3 import AudioToMp3
...
class TestAudioToMp3:
    ...
    def test_node_initialization(self, mock_node_init):
        ...
        with patch.object(AudioToMp3, 'create_subscription'), \
    ...
    def test_subscription_creation(self, mock_node_init):
        ...
        with patch.object(AudioToMp3, 'create_subscription', create_sub_mock), \
             patch.object(AudioToMp3, 'get_logger', return_value=get_logger_mock):
    ...
    def test_audio_callback(self):
        ...
        with patch('rclpy.node.Node.__init__'), \
             patch.object(AudioToMp3, 'create_subscription'), \
             patch.object(AudioToMp3, 'get_logger'):
            ...
            np.testing.assert_array_equal(node.audio_buffer[0], np.array(test_audio_data, dtype=np.float32))
    ...
    def test_multiple_audio_callbacks(self):
        ...
        with patch('rclpy.node.Node.__init__'), \
            ...
            for i, chunk_data in enumerate(test_chunks):
                np.testing.assert_array_equal(node.audio_buffer[i], np.array(chunk_data, dtype=np.float32))
...
class TestAudioToMp3EdgeCases:
    ...
    def test_different_sample_rates(self, sample_rate):
        ...
        from audio_stream_manager.save_mp3 import SAMPLE_RATE
