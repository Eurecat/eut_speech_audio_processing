"""
Unit tests for AudioCapturingNode class.
...
from audio_stream_manager.audio_capturing_automatic_device import AudioCapturingNode, SuppressStderr
...
class TestAudioCapturingNode:
    ...
    def test_parameter_declarations(self, mock_node_init, mock_thread):
        ...
        with patch.object(AudioCapturingNode, 'create_publisher'), \
            ...
            with patch.object(AudioCapturingNode, 'declare_parameter', declare_param_mock):
                node = AudioCapturingNode()
    ...
    def test_get_device_list(self, mock_query_devices, mock_node_init, mock_thread):
        ...
        with patch.object(AudioCapturingNode, 'declare_parameter'), \
            ...
            available_devices = node.get_available_input_devices(mock_devices)
    ...
    def test_find_device_by_name(self, mock_query_devices, mock_node_init, mock_thread):
        ...
        with patch.object(AudioCapturingNode, 'declare_parameter'), \
            ...
            node = AudioCapturingNode()
            node.devices = mock_devices
            # Set up available_devices as would be done in setup_working_device
            node.available_devices = node.get_available_input_devices(mock_devices)
            
            # Test finding existing device using correct method
            device_indices = node.find_devices_by_name("DJI")
            assert 0 in device_indices
            
            # Test finding non-existent device
            device_indices = node.find_devices_by_name("NonExistent")
    ...
    @pytest.mark.parametrize("channels,chunk_size", [
        (1, 512),
        (1, 1024),
        (2, 512),
        (2, 1024),
...
@patch('audio_stream_manager.audio_capturing_automatic_device.threading.Thread')
class TestAudioCapturingNodeEdgeCases:
    ...
    def test_no_audio_devices_available(self, mock_query_devices, mock_node_init, mock_thread):
        ...
        with patch.object(AudioCapturingNode, 'declare_parameter'), \
            ...
            node = AudioCapturingNode()
            node.devices = []
            available_devices = node.get_available_input_devices([])
