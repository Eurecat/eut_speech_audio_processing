...
from launch.actions import OpaqueFunction, LogInfo, SetEnvironmentVariable, DeclareLaunchArgument, TimerAction
...
def _setup(context, *args, **kwargs):
    ...
    if enable_vad.lower() == 'true':
        ...
        new_py_path_vad = site_pkgs_vad if not existing else f"{site_pkgs_vad}{os.pathsep}{existing}"
    ...
    if enable_diarization.lower() == 'true':
        ...
        existing = os.environ.get("PYTHONPATH", "")
        new_py_path_diarization = site_pkgs_diarization if not existing else f"{site_pkgs_diarization}{os.pathsep}{existing}"
        diarization_config_file = os.path.join(config_dir, 'config', 'diarization_params.yaml')
        
        log_messages.extend([
            LogInfo(msg=f"[speech_recognition] Diarization: Using AI venv: {VENV_PATH_DIARIZATION}"),
            LogInfo(msg=f"[speech_recognition] Diarization: Injecting site-packages: {site_pkgs_diarization}"),
            LogInfo(msg=f"[speech_recognition] Diarization: Loading config from: {diarization_config_file}"),
            LogInfo(msg=f"[speech_recognition] Diarization: Will start with {diarization_delay} second delay (starts first)"),
        ])
    ...
    if enable_asr.lower() == 'true':
        ...
        existing = os.environ.get("PYTHONPATH", "")
        new_py_path_asr = site_pkgs_asr if not existing else f"{site_pkgs_asr}{os.pathsep}{existing}"
        ...
        log_messages.extend([
            ...
            LogInfo(msg=f"[speech_recognition] ASR: Will start with {asr_delay} second delay (after Diarization)"),
        ])
